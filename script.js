...
const wineList = [
    ...
    },
    {
        ...
        imageUrl: "https://i.imgur.com/bZJXPfl.jpeg"
    },
      {
...
function showScreen(screenId) {
    ...
    if (screenId === 'home-screen') {
        displayDiscountInfo();
    } else if (screenId === 'beer-screen') {
        displayBeerInfo();
    } else if (screenId === 'chuhai-screen') {
        displayChuhaiInfo();
        initializeChuhaiFilters();
        displaySceneExplanations();
        filterChuhai();
    } else if (screenId === 'wine-screen') {
        displayWineInfo();
        displayBeaujolaisInfo();
        filterWines();
    } else if (screenId === 'sake-screen') {
        displaySakeInfo();
    } else if (screenId === 'shochu-screen') {
        displayShochuInfo();
    } else if (screenId === 'whisky-screen') {
        displayWhiskyInfo();
    }
}
...
function displayBeerInfo() {
    ...
    let html = `
        <h3>ビールの基礎知識</h3>
        <p>ビールは、主に麦芽、ホップ、水、酵母を原料として作られる発酵飲料です。</p>
        
        <h4>主なビールの種類</h4>
        <ul>
            ${beerInfo.types.map(type => `
                <li>
        ...
        <p>代表的な銘柄: ${beerInfo.craftBeer.examples.join(', ')}</p>

        <h4>IPA（インディア・ペールエール）</h4>
        <p>${beerInfo.ipaInfo.description}</p>
        <p>歴史: ${beerInfo.ipaInfo.history}</p>
        <p>特徴:</p>
        <ul>
        ...
        <p>機能: ${beerInfo.spinner.function}</p>
        <p>メリット:</p>
        <ul>
...
function displayWineInfo() {
    ...
    wineInfo.innerHTML = `
        ...
        <ul>
            <li>赤ワイン：黒ブドウの果皮や種子と共に発酵させて作られます。</li>
            <li>白ワイン：主に白ブドウから作られますが、黒ブドウからも作ることができます。</li>
            <li>ロゼワイン：赤ワインの製法で、果皮との接触時間を短くして作られます。</li>
            <li>スパークリングワイン：二次発酵により炭酸ガスを含んだワインです。</li>
        </ul>
        <h3>ワインの楽しみ方</h3>
        <p>ワインを楽しむためのいくつかのポイントを紹介します：</p>
        <ul>
            <li>適切な温度で飲む：赤ワインは16-18℃、白ワインは8-10℃が理想的です。</li>
            <li>グラスの選び方：赤ワインは大きめ、白ワインは小さめのグラスを使います。</li>
            <li>デカンタージュ：赤ワインは空気に触れさせることで香りが開きます。</li>
            <li>料理との相性：赤ワインは肉料理、白ワインは魚料理と相性が良いです。</li>
        </ul>
    `;
}

// ボジョレーヌーボー情報表示
function displayBeaujolaisInfo() {
    const beaujolaisInfo = document.getElementById('beaujolais-nouveau-info');
    beaujolaisInfo.innerHTML = `
        <h3>ボージョレ・ヌーボーについて</h3>
        ...
        <p><strong>専門用語：</strong></p>
        <ul>
            <li><em>マセラシオン・カルボニック</em>：ボージョレ・ヌーボーの製法で、ブドウを潰さずに密閉タンクで発酵させる手法</li>
            <li><em>プリムール</em>：新酒ワインの総称。ボージョレ・ヌーボーもこれに含まれる</li>
            <li><em>AOC</em>：原産地統制呼称。ボージョレ・ヌーボーは「ボージョレAOC」または「ボージョレ・ヴィラージュAOC」</li>
        </ul>
        <p><strong>楽しみ方：</strong></p>
        <ul>
            <li>軽い前菜や鶏肉料理、チーズとの相性が良い</li>
            <li>開封後はなるべく早めに飲み切るのがおすすめ</li>
            <li>解禁日に合わせてパーティーを開くのも人気</li>
        </ul>
    `;
}
...
function displaySakeInfo() {
    ...
    let html = `
        ...
        <h4>日本酒の味わい</h4>
        <ul>
            ${sakeInfo.tastes.map(taste => `<li><strong>${taste.name}:</strong> ${taste.description}</li>`).join('')}
        </ul>

        <h4>北海道の代表的な地酒</h4>
        <ul>
            ${sakeInfo.hokkaido_sake.map(sake => `
                <li><strong>${sake.name}</strong> (${sake.brewery}, ${sake.location}): ${sake.feature}</li>
            `).join('')}
        </ul>

        <h4>全国の有名な地酒</h4>
        <ul>
            ${sakeInfo.famous_regional_sake.map(sake => `
                <li><strong>${sake.prefecture}:</strong> ${sake.name} - ${sake.feature}</li>
            `).join('')}
        </ul>
...
function displayShochuInfo() {
    ...
    let html = `
        <h3>${shochuInfo.comparison.title}</h3>
        <table>
            <tr>
                <th>比較項目</th>
                <th>甲類焼酎</th>
        ...
        <h3>代表的な銘柄</h3>
        <p><strong>甲類焼酎:</strong> ${shochuInfo.types[0].examples.join(', ')}</p>
        <p><strong>乙類焼酎:</strong> ${shochuInfo.types[1].examples.join(', ')}</p>

        <h3>焼酎の種類</h3>
        <ul>
            ${shochuInfo.types.slice(2).map(type => `
                <li>
                    <strong>${type.name}:</strong> ${type.description}
        ...
        <h3>蒸留方法</h3>
        <ul>
            ${shochuInfo.distillationMethods.map(method => `<li><strong>${method.name}:</strong> ${method.description}</li>`).join('')}
        </ul>
    `;
    container.innerHTML = html;
}
...
function filterWines() {
    ...
    const filteredWines = wineList.filter(wine => 
        ...
        (!taste || (taste === "甘口" && (wine.taste === "甘口" || wine.taste === "やや甘口")) || wine.taste === taste) && 
        (!price || wine.price <= parseInt(price))
    );
...
function displayWineResults(wines) {
    ...
    resultsContainer.innerHTML = wines.map(wine => `
        <div class="wine-card">
            ...
            <p>価格: ${wine.price}円</p>
            <p>${wine.salesPoint}</p>
            ${wine.imageUrl ? `<img src="${wine.imageUrl}" alt="${wine.name}" style="max-width: 100px;">` : ''}
        </div>
    `).join('');
}
...
function initializeChuhaiFilters() {
    ...
    const makers = [...new Set(chuhaiList.map(chuhai => chuhai.maker))];
...
function filterChuhai() {
    ...
    const filteredChuhai = chuhaiList.filter(chuhai => 
        (!maker || chuhai.maker === maker) &&
        (!sugarContent || chuhai.sugarContent === sugarContent) &&
        (chuhai.alcohol >= minAlcohol && chuhai.alcohol <= maxAlcohol) &&
        (!scene || chuhai.scene.includes(scene)) &&
        (searchTerm === '' || chuhai.name.toLowerCase().includes(searchTerm) || chuhai.description.toLowerCase().includes(searchTerm))
    );
...
function displayChuhaiResults(results) {
    ...
    chuhaiListElement.innerHTML = results.map(chuhai => `
        <div class="chuhai-card" data-jan="${chuhai.janCode}">
            ${chuhai.imageUrl ? `<img src="${chuhai.imageUrl}" alt="${chuhai.name}" style="max-width: 100px;">` : ''}
            <h3>${chuhai.name}</h3>
            <p>メーカー: ${chuhai.maker}</p>
            <p>アルコール度数: ${chuhai.alcohol}%</p>
            <p>容量: ${chuhai.volume}</p>
            <p>糖分: ${chuhai.sugarContent}</p>
            <p>おすすめシーン: ${chuhai.scene.join(', ')}</p>
            <p>説明: ${chuhai.description}</p>
        </div>
    `).join('');
}

function showModal(chuhai) {
    const modal = document.getElementById('modal');
    const modalContent = document.getElementById('modalContent');
    modalContent.innerHTML = `
        <h2>${chuhai.name}</h2>
        ${chuhai.imageUrl ? `<img src="${chuhai.imageUrl}" alt="${chuhai.name}" style="max-width: 200px;">` : ''}
        <p>メーカー: ${chuhai.maker}</p>
        <p>アルコール度数: ${chuhai.alcohol}%</p>
        <p>容量: ${chuhai.volume}</p>
        <p>糖分: ${chuhai.sugarContent}</p>
...
function displaySceneExplanations() {
    ...
    sceneExplanationsElement.innerHTML = Object.entries(chuhaiScenes).map(([scene, info]) => `
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('chuhaiList').addEventListener('click', (e) => {
        const card = e.target.closest('.chuhai-card');
        if (card) {
            const janCode = card.dataset.jan;
            const chuhai = chuhaiList.find(c => c.janCode === janCode);
            showModal(chuhai);
        }
    });
